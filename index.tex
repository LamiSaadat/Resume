...
\section{Experience}
  \resumeSubHeadingListStart
    \resumeSubheading
      {Full Stack Software Engineer}{March 2023 -- Present}
      {multplAI (Stealth Startup)} {Remote, Toronto, ON}
    \begin{itemize}[leftmargin=0, label={}]
        \small{\item{
         {Python, Flask, AWS, Lambda, EC2, API Gateway, Cloudwatch, Github Actions, Python Unittest} \\
        }}
    \end{itemize}
    % \begin{itemize}[leftmargin=0.15in, label={}]
    %     \small{\item{
    ...
    \vspace{4pt}
      \resumeItemListStart
        \resumeItem{Architected and implemented a serverless backend platform utilizing Python and Flask on AWS Lambda functions and API Gateway, resulting in improved scalability and reduced infrastructure costs.}
      \resumeItem{Led the development of automated tests for all backend endpoints, achieving over 89\% code coverage and ensuring the reliability of the platform.}
        \resumeItem{Implemented NextJS 13's new App Router and Server Side Rendering, resulting in a 57\% reduction in initial page load times compared to the original architecture.}
        \resumeItem{Developed and implemented a scalable and efficient job scraping platform utilizing Scrapy and Scrapeops, resulting in a scraping success rate of over 99\% on various job sites.}
        \resumeItem{Utilized Github actions to implement CI/CD practices, reducing deployment time by an average of 40 minutes per release.}
        \resumeItem{Developed a Chrome plugin enabling seamless autofill of job application forms by integrating with a backend AI-powered API, streamlining the application process.}
      \resumeItemListEnd
